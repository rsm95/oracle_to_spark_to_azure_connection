...
df2 = df2.select(df.KEY1.alias("BGIITerritory"), df.KEY2.alias("BGIICoverage"), \
...
df3 = df3.select(df.FACTOR.alias("FactorB1LC"))
df3.printSchema()
df3.show()
...
df4 = df4.select(df.KEY1.alias("Coverage"), df.FACTOR.alias("FactorC2LC"))
df4.printSchema()
df4.show()
...
df5 = df5.select(df.KEY1.alias("Class_code"), df.KEY2.alias("Coverage85LC"), \
...
df6 = df6.select(df.KEY1.alias("Territory"), df.FACTOR.alias("Factor85TERR"))
df6.printSchema()
df6.show()
...
df6.write \
    .format("jdbc") \
    .option("url", jdbc_url) \
    .option("dbtable", "table5_85TerrMultLC") \
    .option("driver", postgres_driver) \
    .mode("overwrite") \
    .save()
